...
import got from 'got'
import { getConfiguredTestServer, TestStrategy, getRegisteredTestServer } from './helpers'
...
test(`should allow login, and add successMessage to session upon logged in`, async () => {
  ...
  server.get(
    ...
    { preValidation: fastifyPassport.authenticate('test', { authInfo: false }) },
    async (request, reply) => {
      void reply.send(request.session['messages'])
    }
  )
  server.post(
    '/login',
...
test(`should add failureMessage to session if failed to log in`, async () => {
  const { server, fastifyPassport } = getConfiguredTestServer()
  server.get('/', async (request, reply) => reply.send(request.session['messages']))
  server.post(
    '/login',
    {
      preValidation: fastifyPassport.authenticate('test', {
