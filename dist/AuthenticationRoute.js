...
class AuthenticationRoute {
    ...
    attemptStrategy(failures, prototype, request, reply) {
        ...
        return new Promise((resolve, reject) => {
            ...
            strategy.fail = function (challengeOrStatus, status) {
                request.log.trace({ name: strategy.name }, 'passport strategy failed');
                let challenge;
            ...
            strategy.redirect = (url, status) => {
                request.log.trace({ name: strategy.name, url }, 'passport strategy redirecting');
                void reply.status(status || 302);
                void reply.redirect(url);
                resolve();
            };
            strategy.pass = () => {
                request.log.trace({ name: strategy.name }, 'passport strategy passed');
                resolve();
            };
            strategy.error = (err) => {
                request.log.trace({ name: strategy.name, err }, 'passport strategy errored');
                if (this.callback) {
                    return resolve(this.callback(request, reply, err));
                }
                reject(err);
            };
            request.log.trace({ name: strategy.name }, 'attempting passport strategy authentication');
