...
type AuthenticateFactoryCallback = (err: null | Error, user?: any, info?: any, statuses?: any | any[]) => void;
  ...
  async function authenticate(request: FastifyRequest, reply: FastifyReply) {
    ...
    function allFailed() {
      ...
      if (authenticateOptions.failWithError) {
        throw new AuthenticationError(http.STATUS_CODES[reply.statusCode]!, rstatus as number);
      }
    ...
    await new Promise((resolve, reject) => {
      (function attempt(i) {
         ...
         * details.
         */
        strategy.success = function (user: any, info: { type: string; message: string }) {
          if (callback) {
